...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::UpdateSurface(
    ...
    if (dstTextureInfo->ShouldUpdateMappedBufferEarly(dst->GetSubresource()))
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::UpdateTexture(
    ...
    if (dstTexInfo->IsAutomaticMip()) {
      for (uint32_t i = 0; i < dstTexInfo->Desc()->ArraySize; i++) {
        if (dstTexInfo->ShouldUpdateMappedBufferEarly(dstTexInfo->CalcSubresource(i, 0)))
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::GetRenderTargetData(
    ...
    if (dstTexInfo->ShouldUpdateMappedBufferEarly(dst->GetSubresource()))
