...
namespace dxvk {
  ...
  VkDeviceSize DxvkBuffer::computeSliceAlignment() const {
    ...
    const auto &coreLimits = devInfo.core.properties.limits;
    const auto &robustness2    = devInfo.extRobustness2;

    VkDeviceSize result = sizeof(uint32_t);

    if (m_info.usage & VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT)
      result = std::max(result, coreLimits.minUniformBufferOffsetAlignment);

    if (m_info.usage & (VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT))
      result = std::max(result, robustness2.robustUniformBufferAccessSizeAlignment);

    if (m_info.usage & VK_BUFFER_USAGE_STORAGE_BUFFER_BIT)
      result = std::max(result, coreLimits.minStorageBufferOffsetAlignment);

    if (m_info.usage & (VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT)) {
      result = std::max(result, coreLimits.minTexelBufferOffsetAlignment);
      result = std::max(result, VkDeviceSize(16));
    }

    if (m_info.usage & (VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT))
      result = std::max(result, robustness2.robustStorageBufferAccessSizeAlignment);

    if (m_info.usage & (VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT)
     && m_info.size > (coreLimits.optimalBufferCopyOffsetAlignment / 2))
      result = std::max(result, coreLimits.optimalBufferCopyOffsetAlignment);
    ...
    if (m_memFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
      result = std::max(result, coreLimits.nonCoherentAtomSize);
